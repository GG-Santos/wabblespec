...
augment_ref: cross-link-document-required-section-v1
---
...
**Reference-type outputs require a cross-references section (AUGMENT: new rule).** When the task-card declares a documentation deliverable of type `reference` — any file destined for `_shared/references/` or explicitly typed `reference` in the task-card — Document must inspect its own output before writing the document receipt.

The inspection check:
```
if deliverable.type == "reference":
    ...
    elif cross_references_section has zero entries:
        emit CROSS_LINK_MISSING
        do not write document-receipt
...
A cross-references section must contain at least one entry. Section header alone (empty section) does not satisfy the requirement.
...
**Treating the section header as satisfying the requirement.** An empty `## Cross-references` heading does not pass the check. At least one entry (link, module reference, or schema pointer) must appear under the heading.

**Applying the check to non-reference outputs.** README updates, inline comments, and CHANGELOG entries follow different conventions. The cross-references check applies only to `_shared/references/` documents or task-card-typed `reference` deliverables.

## Not tested

Augment cannot verify that the CROSS_LINK_MISSING check fires correctly for all reference document shapes. Benchmark validates against fixtures. The check assumes `deliverable.type` is declared in the task-card. Task cards that do not explicitly declare deliverable type require the check to fall back to path-based detection (file path starts with `_shared/references/`).
