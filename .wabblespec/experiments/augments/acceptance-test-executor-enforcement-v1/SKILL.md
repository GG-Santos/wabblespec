...
augment_ref: acceptance-test-executor-enforcement-v1
---
...
### After all waves complete — module-build acceptance gate (AUGMENT: new step)
...
If `task_type` is anything other than `module-build`, skip this check entirely and write `execution-receipt.json` as usual.
...
Augment cannot verify that the ACCEPTANCE_NOT_COVERED check fires correctly for all task-card shapes. Benchmark validates against fixtures. The check assumes `task_type` is a top-level field in task-card.md. Task cards that declare task type in a non-standard location will require task-card schema alignment before this check is reliable.
