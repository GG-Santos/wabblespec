# Document (Augment) — Acceptance Criteria

Includes all original acceptance criteria plus new criteria for the reference-type cross-references gate.
...
## AT-DOC-05: PASS — reference output with cross-references section present (AUGMENT)

**Given** the task-card declares a `reference`-type deliverable (path in `_shared/references/` or type explicitly `reference`)
**And** the produced file contains a `## Cross-references` or `## See also` section with at least one entry
...
## AT-DOC-06: BLOCK — reference output with no cross-references section (AUGMENT)
...
## AT-DOC-07: BLOCK — reference output with empty cross-references section (AUGMENT)

**Given** the task-card declares a `reference`-type deliverable
**And** the produced file contains a `## Cross-references` section header but no entries beneath it
**When** Document inspects the output
**Then** Document emits CROSS_LINK_MISSING (an empty section header does not satisfy the requirement)
**Then** Document does not write the document receipt

---

## AT-DOC-08: PASS — non-reference output bypasses cross-references check (AUGMENT)
...
**Given** a reference deliverable where the cross-references section is absent or empty
**Then** Document does not write the document receipt regardless of task urgency
**Then** at least one cross-reference entry must be added before Document can receipt the task complete
