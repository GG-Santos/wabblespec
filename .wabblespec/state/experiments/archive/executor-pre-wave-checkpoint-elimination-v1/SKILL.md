...
If `context7.available: true` in `.wabblespec/runtime/runtime-state.json`, and the wave implementation involves a named library or framework not covered by `_shared/dev/`, use context7 to fetch the relevant API surface before implementing:
...
See `_shared/references/context7-integration.md` for full call patterns and receipt field spec.
...
After the wave receipt is written, write `.wabblespec/session/checkpoints/checkpoint-wave-<N>.json` conforming to `_shared/schemas/wave-checkpoint.schema.json`. Required fields:
