...
## Gate 10: Accessibility Minimum Compliance

**Check:** App meets the declared accessibility tier. Tier AA (minimum) requires: screen reader navigation, Dynamic Type / font scaling, minimum touch targets, and minimum contrast.

**Method:**

**VoiceOver (iOS):**
1. Settings > Accessibility > VoiceOver > On
2. Navigate all screens with swipe-left/right and double-tap
3. Every interactive element must announce a meaningful label
4. No unlabeled buttons ("button", "image" with no context)

**TalkBack (Android):**
1. Settings > Accessibility > TalkBack > On
2. Navigate all screens with swipe gestures
3. Every interactive element must have a content description

**Dynamic Type (iOS):**
```bash
# Run app with Largest Accessibility text size
# Simulator: Settings > Accessibility > Display & Text Size > Larger Text → max
# Verify: no clipped text, no overlapping elements, all labels readable
```

**Font scale (Android):**
```bash
adb shell settings put system font_scale 2.0
# Verify app at 200% font scale — no broken layouts
adb shell settings put system font_scale 1.0  # reset
```

**Touch targets:**
```bash
# iOS: Settings > Developer > Show Accessibility Frames (Simulator)
# Android: adb shell uiautomator dump — check bounds for minimum 48x48dp
adb shell uiautomator dump /sdcard/ui.xml
adb pull /sdcard/ui.xml
# Inspect bounds for interactive elements
```

**Contrast:**
- Body text: minimum 4.5:1 contrast ratio against background
- Large text (18pt+ or 14pt bold): minimum 3:1
- Use Accessibility Inspector (iOS) or Accessibility Scanner (Android) to audit

**Pass:** All interactive elements have meaningful screen reader labels. Text scales to maximum system font size without layout breakage. All interactive elements meet minimum touch target size. Body text contrast ratio ≥ 4.5:1.
**Fail:** Any unlabeled interactive element. Text clipped or overlapping at maximum font scale. Touch target below platform minimum. Body text below 4.5:1 contrast.

**Skip condition:** Document with justification if target platform does not support screen reader (embedded/kiosk device). Tier must be declared in design-document.md. Accessibility tier below AA requires Attestation sign-off.
...
| 10 | Accessibility minimum compliance | Yes (skip requires Attestation) |

All applicable gates are blocking. No Delivery wave proceeds with any gate in FAIL state. Skipped gates must be documented with justification and signed off via Attestation.
