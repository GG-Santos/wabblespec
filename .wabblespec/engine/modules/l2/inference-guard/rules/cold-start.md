...
Read `.wabblespec/state/plans/task-card.md`. Extract:
...
Read `.wabblespec/state/receipts/` for a `gateway-security-spec-receipt-*.json` written this session. If none found: InferenceGuard does not activate. Write non-activation receipt with `reason: gateway_inactive`.
