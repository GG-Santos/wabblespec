...
Create named ranges for any value referenced from more than one sheet or linked from an external document (a pitch deck referencing model output, for example). Named ranges survive row/column insertions and make cross-sheet formulas readable.

```python
# Define named ranges for key assumptions
wb.create_named_range('WACCRate', ws, '$D$2')
wb.create_named_range('RevenueGrowth', ws, '$C$3')

# Reference by name from another sheet — survives row inserts
calc = wb.create_sheet("DCF")
calc['B5'] = '=WACCRate'
calc['C5'] = '=Inputs!C2*(1+RevenueGrowth)'
```

## Balance Checks Tab

Any financial model delivered as output must include a Checks sheet as the last tab. Each check returns the string `"TRUE"` or `"FALSE"` (not a number) so readers see a binary result at a glance. A single `"FALSE"` blocks delivery.

Mandatory checks for financial models:
- Balance sheet ties (Assets = Liabilities + Equity)
- Cash flow reconciles (Opening cash + net cash flows = Closing cash)
- Totals cross-foot (sum of parts = stated total)

```python
checks = wb.create_sheet("Checks")
checks['A1'] = 'Check'
checks['B1'] = 'Result'
checks['A2'] = 'BS balance (Assets = Liabilities + Equity)'
checks['B2'] = '=IF(FinancialModel!C50=FinancialModel!C51+FinancialModel!C52,"TRUE","FALSE")'
checks['A3'] = 'CF reconciles (Opening + Net = Closing)'
checks['B3'] = '=IF(FinancialModel!D80+FinancialModel!D81=FinancialModel!D82,"TRUE","FALSE")'
wb.move_sheet("Checks", offset=len(wb.sheetnames))
```

Deliver only when all Checks cells show `"TRUE"`.
