...
STM is applied to the prose content of defense findings only — not to code blocks or test cases. Run `.wabblespec/engine/shared/scripts/stm-pipeline.py` with transforms `["hedge_reducer", "direct_mode"]` on each finding's prose fields before writing the Blue receipt.
