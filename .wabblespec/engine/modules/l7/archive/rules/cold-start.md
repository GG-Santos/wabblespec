...
Condition: No `verifier-receipt.json` (or delivery-receipt) in `.wabblespec/receipts/` for the current task.
...
Condition: `.wabblespec/archive/` does not exist.
Detection: Directory read returns 404.
Action: Create `.wabblespec/archive/` and `receipt-index.json` with empty entries. This is normal on first archive.
