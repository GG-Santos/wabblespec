...
WabbleSpecMemory's tunnel primitive in ``palace_graph.py`` connects rooms
...
``~/.wabblespec_memory/`` so the records survive across mines and are
...
logger = logging.getLogger("wabblespec_memory_hallways")

def _memory_artifact_path(filename: str) -> str:
    memory_root = os.environ.get("WABBLESPEC_MEMORY_PATH")
    if memory_root:
        return os.path.join(memory_root, filename)
    return os.path.join(os.path.expanduser("~"), ".wabblespec_memory", filename)


# Persistence target. Mirrors ``palace_graph`` so the storage pattern is
# uniform across the two related primitives. Tests override this via
# ``monkeypatch.setattr(hallways, "_HALLWAY_FILE", tmp_path/...)``.
_HALLWAY_FILE = _memory_artifact_path("hallways.json")
