...
This module is used as the primary signal in `memory init`. The regex
...
def _merge_detected(primary: dict, secondary: dict, drop_secondary_uncertain: bool = False) -> dict:
    ...
    Dedup is case-insensitive so "wabblespec_memory" (manifest name) absorbs "WabbleSpec Memory"
...
def discover_entities(
    ...
    The expected shape is the dict written by ``memory init`` to
    ``<palace>/.wabblespec_memory/origin.json`` (see ``corpus_origin.py``).
    """
    ...
    from memory.convo_scanner import is_claude_projects_root, scan_claude_projects
    ...
    if is_claude_projects_root(root_path):
        ...
        # so a `pyproject.toml` name like `wabblespec_memory` and a Claude Code
        # `cwd` variant like `WabbleSpec Memory` collapse into one entry — matches
    ...
    if llm_provider is not None:
        from memory.llm_refine import collect_corpus_text, refine_entities
    ...
    if corpus_origin is not None:
        from memory.entity_detector import _apply_corpus_origin
