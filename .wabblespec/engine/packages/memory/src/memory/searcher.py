...
def _warn_if_legacy_metric(col) -> None:
    ...
    need ``memory repair`` to rebuild with the correct metric.
    ...
    print(
        ...
        "          Run `memory repair` to rebuild the index with the correct metric.",
        file=_sys.stderr,
    )
...
def search(query: str, palace_path: str, wing: str = None, room: str = None, n_results: int = 5):
    ...
    if not os.path.isdir(palace_path):
        ...
        print("  Run: memory init <dir> then memory mine <dir>")
    ...
    # see via `wabblespec_memory_search`.
    hits = [
    ...
    for h in hits:
        ...
        if len(expanded) > MAX_HYDRATION_CHARS:
            expanded = (
                ...
                "Use wabblespec_memory_get_drawer for full content.]"
            )
        h["text"] = expanded
