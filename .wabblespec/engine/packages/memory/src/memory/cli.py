#!/usr/bin/env python3
"""
WabbleSpec Memory — Give your AI a memory. No API key required.

Three ways to ingest:
  Projects:      memory mine ~/projects/my_app                  (code, docs, notes)
  Conversations: memory mine <convo-dir> --mode convos          (Claude Code, Claude.ai, ChatGPT, Slack exports)
  Documents:     memory mine <docs-dir> --mode extract          (PDF, DOCX, PPTX, XLSX, RTF, EPUB — requires memory[extract])

Same palace. Same search. Different ingest strategies.

Commands:
    memory init <dir>                  Detect rooms from folder structure
    memory split <dir>                 Split concatenated mega-files into per-session files
    memory mine <dir>                  Mine project files (default)
    memory mine <dir> --mode convos    Mine conversation exports
    memory mine <dir> --mode extract   Mine binary office documents (PDF/DOCX/etc.)
    memory search "query"              Find anything, exact words
    memory mcp                         Show MCP setup command
    memory wake-up                     Show L0 + L1 wake-up context
    memory wake-up --wing my_app       Wake-up for a specific project
    memory status                      Show what's been filed

Examples:
    memory init ~/projects/my_app
    memory mine ~/projects/my_app
    memory mine ~/.claude/projects/-Users-you-Projects-my_app --mode convos --wing my_app
    memory search "why did we switch to GraphQL"
    memory search "pricing discussion" --wing my_app --room costs
"""
...
from .config import DEFAULT_MAX_CHUNKS_PER_FILE, WabbleSpecMemoryConfig
from .corpus_origin import detect_origin_heuristic, detect_origin_llm
from .llm_client import LLMError, get_provider
from .version import __version__


_WABBLESPEC_MEMORY_PROJECT_FILES = ("memory.yaml", "entities.json")
...
def _gather_origin_samples(project_dir) -> list:
    ...
    Skips wabblespec_memory's own per-project artifacts (``entities.json``,
    ``memory.yaml``) so a re-run of ``memory init`` produces the
    ...
    for filepath in files:
        if filepath.name in _WABBLESPEC_MEMORY_PROJECT_FILES:
            continue
...
def _run_pass_zero(project_dir, palace_dir, llm_provider) -> dict:
    ...
    result to ``<palace>/.wabblespec_memory/origin.json``.
    ...
    origin_path = Path(palace_dir).expanduser() / ".wabblespec_memory" / "origin.json"
    try:
...
def _ensure_wabblespec_memory_files_gitignored(project_dir) -> bool:
    """If project_dir is a git repo, ensure WabbleSpec Memory's per-project files
    ...
    `memory init` writes memory.yaml + entities.json into the
    ...
    missing = [p for p in _WABBLESPEC_MEMORY_PROJECT_FILES if p not in existing_lines]
    if not missing:
        return False
    prefix = "" if not existing or existing.endswith("\n") else "\n"
    block = prefix + "\n# WabbleSpec Memory per-project files (issue #185)\n" + "\n".join(missing) + "\n"
...
def cmd_init(args):
    ...
    # flag and always used ~/.memory. Mirror the env-var pattern used by
    ...
    if getattr(args, "palace", None):
        os.environ["WABBLESPEC_MEMORY_PATH"] = os.path.abspath(os.path.expanduser(args.palace))

    cfg = WabbleSpecMemoryConfig()
            ...
            if ok:
                ...
                if candidate.is_external_service:
                    print(
                        ...
                        f"WabbleSpec Memory does not control how the provider logs, "
    ...
    # <palace>/.wabblespec_memory/origin.json and supplies corpus context to the
    ...
    if total > 0:
        ...
        if confirmed["people"] or confirmed["projects"] or confirmed.get("topics"):
            ...
            # ``memory.yaml`` so the miner's tunnel lookup hits the
    ...
    _ensure_wabblespec_memory_files_gitignored(args.dir)
...
def _maybe_run_mine_after_init(args, cfg) -> None:
    ...
    if not auto_mine:
        ...
        if answer not in ("", "y", "yes"):
            print(f"\n  Skipped. Run `memory mine {shlex.quote(project_dir)}` when ready.")
            return
...
def cmd_mine(args):
    palace_path = os.path.expanduser(args.palace) if args.palace else WabbleSpecMemoryConfig().palace_path
    include_ignored = []
    for raw in args.include_ignored or []:
        include_ignored.extend(part.strip() for part in raw.split(",") if part.strip())

    # --redetect-origin re-runs corpus_origin on the current corpus state
    # and overwrites <palace>/.wabblespec_memory/origin.json before mining proceeds.
    # Heuristic-only by design — full LLM detection lives on `memory init`.
...
def cmd_sync(args):
    ...
    print(f"\n{'=' * 55}")
    print("  WabbleSpec Memory Sync — Gitignore-aware drawer prune")
    print(f"{'=' * 55}")
    print(f"  Memory store:   {palace_path}")
    if args.wing:
    ...
    try:
        ...
        print(f"memory: sync failed: {exc}", file=sys.stderr)
        sys.exit(1)
...
def cmd_repair(args):
    ...
    print(f"\n{'=' * 55}")
    print(" WabbleSpec Memory Repair")
    print(f"{'=' * 55}\n")
    print(f"  Memory store: {palace_path}")
...
def _format_command(parts: list[str]) -> str:
    return " ".join(_quote_cmd_arg(part) for part in parts)


def _current_python_has_server_dependencies() -> bool:
    return importlib.util.find_spec("chromadb") is not None


def cmd_mcp(args):
    """Show how to wire WabbleSpec Memory into MCP-capable hosts."""
    base_server_parts = _server_command_parts()

    if args.palace:
        resolved_palace = str(Path(args.palace).expanduser())
        server_parts = [*base_server_parts, "--palace", resolved_palace]
    else:
        server_parts = base_server_parts

    server_cmd = _format_command(server_parts)
    base_server_cmd = _format_command(base_server_parts)

    print("WabbleSpec Memory MCP quick setup:")
    print(f"  claude mcp add memory -- {server_cmd}")
    print(f"  codex mcp add memory -- {server_cmd}")
    print("\nRun the server directly:")
    print(f"  {server_cmd}")

    if (
        base_server_parts
        and base_server_parts[0] == sys.executable
        and not _current_python_has_server_dependencies()
    ):
        print("\nDependency note:")
        print(
            "  This Python cannot import chromadb. Install packages/memory in this "
            "environment or run the command from one that has the shared package "
            "dependencies."
        )

    if not args.palace:
        print("\nOptional custom palace:")
        custom_cmd = f"{base_server_cmd} --palace /path/to/palace"
        print(f"  claude mcp add memory -- {custom_cmd}")
        print(f"  codex mcp add memory -- {custom_cmd}")
        print(f"  {custom_cmd}")
...
def cmd_compress(args):
    ...
    col = _open_collection_or_explain(palace_path, collection_name="wabblespec_drawers")
    if col is None:
        sys.exit(1)
    ...
    if not args.dry_run:
        try:
            ...
            print(
                f"  Stored {len(compressed_entries)} compressed drawers in 'wabblespec_closets' collection."
            )
        except Exception as e:
...
def _reconfigure_stdio_utf8_on_windows():
    """Decode stdio as UTF-8 on Windows for the primary `memory` CLI.

    Thin wrapper around the shared helper in ``memory._stdio``. The CLI
    overrides stdout/stderr to ``replace`` because ``memory search``
...
def main():
    """CLI entry point for the ``memory`` console script.
    ...
    # memory/__init__.py already protects this process from the same
    # ABI mismatch; here we extend the protection to children.
    os.environ.pop("PYTHONPATH", None)

    _reconfigure_stdio_utf8_on_windows()

    version_label = f"WabbleSpec Memory {__version__}"
    parser = argparse.ArgumentParser(
        description="WabbleSpec Memory — Give your AI a memory. No API key required.",
    ...
    parser.add_argument(
        "--palace",
        default=None,
        help="Where the palace lives (default: from ~/.wabblespec_memory/config.json or ~/.wabblespec_memory/palace)",
    )
    ...
    p_init.add_argument(
        ...
        help=(
            ...
            "(WABBLESPEC_MEMORY_ENTITY_LANGUAGES env var or config.json), or 'en'. "
    ...
    p_mine.add_argument(
        "--agent",
        default="memory",
        help="Your name — recorded on every drawer (default: memory)",
    )
    ...
    p_mine.add_argument(
        ...
        help=(
            ...
            "<palace>/.wabblespec_memory/origin.json. Useful when the corpus has grown "
            "since `memory init` and the stored origin may be stale. "
            "Heuristic-only (no LLM call) — re-run `memory init --llm` for "
            "Tier 2 refinement."
        ),
    )
    p_mine.add_argument(
    ...
    p_mine.add_argument(
        ...
        help=(
            ...
            f"summary counter. Default {DEFAULT_MAX_CHUNKS_PER_FILE} "
            f"(or WABBLESPEC_MEMORY_MAX_CHUNKS_PER_FILE). Set 0 to disable. Lower this on "
    ...
    sub.add_parser(
        "mcp",
        help="Show MCP setup command for connecting WabbleSpec Memory to your AI client",
    )
