#!/usr/bin/env python3
"""
onboarding.py — WabbleSpec Memory first-run setup.

Asks the user:
  1. How they're using WabbleSpec Memory (work / personal / combo)
...
Seeds the entity_registry with confirmed data so WabbleSpec Memory knows your world
from minute one — before a single session is indexed.

Usage:
    python3 -m memory.onboarding
    or: memory init
"""

from pathlib import Path
from memory.entity_registry import EntityRegistry
from memory.entity_detector import detect_entities, scan_for_detection
...
def _ask_mode() -> str:
    _header("Welcome to WabbleSpec Memory")
    print("""
  WabbleSpec Memory is a personal memory system. To work well, it needs to know
...
""")
    print("  How are you using WabbleSpec Memory?")
    print()
...
def _ask_projects(mode: str) -> list:
    ...
    _hr()
    print("""
  What are your main projects? (These help WabbleSpec Memory distinguish project
...
def _ask_wings(mode: str) -> list:
    ...
    _hr()
    print(f"""
  Wings are the top-level categories in your memory store.
...
def _warn_ambiguous(people: list) -> list:
    ...
    """
    from memory.entity_registry import COMMON_ENGLISH_WORDS
...
def _generate_aaak_bootstrap(
    ...
    """
    wabblespec_memory_dir = Path(config_dir) if config_dir else Path.home() / ".wabblespec_memory"
    wabblespec_memory_dir.mkdir(parents=True, exist_ok=True)
    ...
    registry_lines = [
        "# AAAK Entity Registry",
        "# Auto-generated by memory init. Update as needed.",
        "",
        "## People",
    ]
    for p in people:
    ...
    (wabblespec_memory_dir / "aaak_entities.md").write_text("\n".join(registry_lines), encoding="utf-8")
    ...
    (wabblespec_memory_dir / "critical_facts.md").write_text("\n".join(facts_lines), encoding="utf-8")
...
def run_onboarding(
  ...
  WabbleSpec Memory will check the context before treating them as person names.
...
""")
    ...
    print("\n  AAAK entity registry: ~/.wabblespec_memory/aaak_entities.md")
    print("  Critical facts bootstrap: ~/.wabblespec_memory/critical_facts.md")
