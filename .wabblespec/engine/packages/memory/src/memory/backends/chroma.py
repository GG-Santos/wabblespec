"""ChromaDB-backed WabbleSpec Memory storage backend (RFC 001 reference implementation)."""
...
def _read_sync_threshold(palace_path: str, collection_name: str) -> int:
    ...
    explicit setting — matches what older wabblespec_memory stores were created
    with before PR #1191.
    """
    db_path = os.path.join(palace_path, "chroma.sqlite3")
    if not os.path.isfile(db_path):
...
def hnsw_capacity_status(palace_path: str, collection_name: str = "wabblespec_drawers") -> dict:
    ...
    try:
        ...
        if divergence > threshold:
            ...
            out["message"] = (
                ...
                "are invisible to vector search. Run `memory repair` to rebuild."
            )
        else:
            out["status"] = "ok"
            out["message"] = (
...
def _sqlite_embedding_count(palace_path: str, collection_name: str) -> Optional[int]:
    ...
    Mirrors :func:`memory.repair.sqlite_drawer_count` but kept in this
...
class ChromaCollection(BaseCollection):
    ...
    direct-backend writers against ``memory mine`` and against each other,
...
class ChromaBackend(BaseBackend):
    """WabbleSpec Memory's default ChromaDB backend.
