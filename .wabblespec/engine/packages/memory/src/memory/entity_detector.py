...
Used by memory init before mining begins.
...
Multi-language support:
    ...
    the ``entity`` section of ``wabblespec_memory/i18n/<lang>.json``. Every public
...
Usage:
    from memory.entity_detector import detect_entities, confirm_entities
...
from memory.i18n import get_entity_patterns
...
# ``wabblespec_memory/i18n/en.json``. Callers that need multi-language behavior should
...
def detect_entities(
    ...
    Args:
        ...
        corpus_origin: Optional corpus-origin context (the dict produced
            by ``memory.corpus_origin`` and persisted to
            ``<palace>/.wabblespec_memory/origin.json`` by ``memory init``).
...
def confirm_entities(detected: dict, yes: bool = False) -> dict:
    ...
    """
    print(f"\n{'=' * 58}")
    print("  WabbleSpec Memory — Entity Detection")
    print(f"{'=' * 58}")
