...
``memory.sources.transforms.<adapter_name>_<transform_name>`` so the
...
# reference under
# ``memory.sources.transforms.<adapter_name>_<transform_name>`` (per the
...
def strip_tool_chrome(text: str) -> str:
    ...
    ``memory.sources.transforms.conversations_strip_tool_chrome``.
    """
    return text
