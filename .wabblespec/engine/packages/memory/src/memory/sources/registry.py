...
``memory.sources`` entry point::

    # pyproject.toml of wabblespec_memory-source-cursor
    [project.entry-points."memory.sources"]
    cursor = "wabblespec_memory_source_cursor:CursorAdapter"

WabbleSpec Memory discovers them at process start. In-tree tests and local
...
(to preserve current ``memory mine <path>`` behavior).
"""
...
_ENTRY_POINT_GROUP = "memory.sources"
