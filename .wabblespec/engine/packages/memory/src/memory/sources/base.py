"""Source adapter contract for WabbleSpec Memory (RFC 002).

Mirrors what ``wabblespec_memory/backends/base.py`` does for the write side: it defines
...
This module is spec scaffolding. The first-party miners (``wabblespec_memory/miner.py``
and ``wabblespec_memory/convo_miner.py``) are migrated onto it in a follow-up PR;
