...
def mine_convos(
    ...
    per-palace flock around it so two concurrent ``memory mine --mode
    convos`` invocations against the same palace can't pile up. This
    mirrors the pattern in :func:`memory.miner.mine`. The lock is
    ...
    WabbleSpecMemoryConfig inside :func:`_mine_convos_impl` so `config.json`
...
def _mine_convos_impl(
    ...
    print(f"\n{'=' * 55}")
    print("  WabbleSpec Memory Mine — Conversations")
    print(f"{'=' * 55}")
    print(f"  Wing:    {wing}")
...
if __name__ == "__main__":
    ...
    mine_convos(sys.argv[1], palace_path=WabbleSpecMemoryConfig().palace_path)
