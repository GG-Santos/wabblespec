# WabbleSpec Memory Search

When the user wants to search their WabbleSpec Memory memories, follow these steps:
...
- wabblespec_memory_search(query, wing, room) -- Primary search tool. Pass the semantic
  query and any wing/room filters.
- wabblespec_memory_list_wings -- Discover all available wings. Use when the user asks
  what categories exist or you need to resolve a wing name.
- wabblespec_memory_list_rooms(wing) -- List rooms within a specific wing. Use to help
  the user navigate or to resolve a room name.
- wabblespec_memory_get_taxonomy -- Retrieve the full wing/room/drawer tree. Use when
  the user wants an overview of their entire memory structure.
- wabblespec_memory_traverse(room) -- Walk the knowledge graph starting from a room.
  Use when the user wants to explore connections and related memories.
- wabblespec_memory_find_tunnels(wing1, wing2) -- Find cross-wing connections (tunnels)
...
If MCP tools are not available, fall back to the CLI:
    ...
    memory search "query" [--wing X] [--room Y]
