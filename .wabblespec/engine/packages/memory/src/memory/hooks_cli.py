"""
Hook logic for WabbleSpec Memory — Python implementation of session-start, stop, and precompact hooks.
...
SAVE_INTERVAL = 15
PALACE_ROOT = Path(os.environ.get("WABBLESPEC_MEMORY_PATH", Path.home() / ".wabblespec_memory"))
STATE_DIR = Path(
    os.environ.get("WABBLESPEC_MEMORY_STATE_DIR", str(PALACE_ROOT / ".runtime" / "hook_state"))
)
...
def _palace_root_exists() -> bool:
    ...
    If ~/.wabblespec_memory/ does not exist, the user has explicitly cleared it.
    ...
    ``~/.wabblespec_memory`` (or a broken symlink) is treated as absent — otherwise
...
def _wabblespec_memory_python() -> str:
    """Return the python interpreter that has memory installed.
    ...
    1. WABBLESPEC_MEMORY_PYTHON env var (explicit override)
    2. Venv python from package install path
    3. Editable install: venv/ sibling to memory/
    4. sys.executable fallback
    """
    # Honor explicit override (used by shell hook wrappers)
    env_python = os.environ.get("WABBLESPEC_MEMORY_PYTHON", "")
    if env_python and os.path.isfile(env_python) and os.access(env_python, os.X_OK):
        return env_python
    # This file lives at <venv>/lib/pythonX.Y/site-packages/memory/hooks_cli.py
    # or <project>/memory/hooks_cli.py (editable install).
    #
    ...
    if len(parents) > 3:
        ...
        if venv_bin.is_file():
            return str(venv_bin)
    # Editable install: assumes project root has a venv/ sibling to memory/
    if len(parents) > 1:
...
STOP_BLOCK_REASON = (
    "AUTO-SAVE checkpoint (WabbleSpec Memory). Save this session's key content:\n"
    "1. wabblespec_memory_diary_write — session summary (what was discussed, "
    "key decisions, current state of work)\n"
    "2. wabblespec_memory_add_drawer — verbatim quotes, decisions, code snippets "
    "(place in appropriate wing and room)\n"
    "3. wabblespec_memory_kg_add — entity relationships (optional)\n"
    "For THIS save, use WabbleSpec Memory MCP tools only (not auto-memory .md files). "
    "Use verbatim quotes where possible. Continue conversation after saving."
)

PRECOMPACT_BLOCK_REASON = (
    "COMPACTION IMMINENT (WabbleSpec Memory). Save ALL session content before context is lost:\n"
    "1. wabblespec_memory_diary_write — thorough session summary\n"
    "2. wabblespec_memory_add_drawer — ALL verbatim quotes, decisions, code, context "
    "(place each in appropriate wing and room)\n"
    "3. wabblespec_memory_kg_add — entity relationships (optional)\n"
    "For THIS save, use WabbleSpec Memory MCP tools only (not auto-memory .md files). "
    "Be thorough — after compaction this is all that survives. "
    "Save everything to WabbleSpec Memory, then allow compaction to proceed."
)
...
def _output(data: dict):
    ...
    If memory.mcp_server is already loaded, reuse its saved real stdout fd.
    ...
    mcp_mod = sys.modules.get("memory.mcp_server") or sys.modules.get(
...
def _get_mine_targets() -> list[tuple[str, str]]:
    ...
    WABBLESPEC_MEMORY_DIR (when set and resolvable) contributes a ``"projects"``
    ...
    An empty list means no WABBLESPEC_MEMORY_DIR ingest should run.
    """
    targets: list[tuple[str, str]] = []
    mempal_dir = os.environ.get("WABBLESPEC_MEMORY_DIR", "")
    if mempal_dir:
...
_MINE_PID_FILE_ENV = "WABBLESPEC_MEMORY_MINE_PID_FILE"
...
# its slot forever.  Set WABBLESPEC_MEMORY_MINE_TIMEOUT_HOURS=0 to disable the
# timeout (slots are reclaimed only when the PID is dead).
_MINE_TIMEOUT_HOURS_ENV = "WABBLESPEC_MEMORY_MINE_TIMEOUT_HOURS"
...
def _mine_slot_timeout_secs() -> float:
    ...
    Reads ``WABBLESPEC_MEMORY_MINE_TIMEOUT_HOURS`` from the environment (float).
...
def _spawn_mine(cmd: list) -> None:
    ...
    ``WABBLESPEC_MEMORY_MINE_PID_FILE`` env var so its cleanup hook can remove
...
def _maybe_auto_ingest():
    """Background-mine WABBLESPEC_MEMORY_DIR (project files) if set.
    ...
    for mine_dir, mode in targets:
        try:
            _spawn_mine([_wabblespec_memory_python(), "-m", "memory", "mine", mine_dir, "--mode", mode])
        except OSError:
            pass


def _mine_sync():
    """Synchronously mine WABBLESPEC_MEMORY_DIR (precompact path).
...
def _desktop_toast(body: str, title: str = "WabbleSpec Memory"):
    """Send a desktop notification via notify-send. Fails silently."""
    try:
        subprocess.Popen(
            ["notify-send", "--app-name=WabbleSpec Memory", "--icon=brain", title, body],
...
def _ingest_transcript(transcript_path: str):
    ...
    try:
        WabbleSpecMemoryConfig()  # validate config loads
    except Exception:
        return
...
def hook_stop(data: dict, harness: str):
    ...
    if str(stop_hook_active).lower() in ("true", "1", "yes"):
        ...
        try:
            ...
            _log(
                f"WARNING: could not import WabbleSpecMemoryConfig for stop guard: {exc}; defaulting to silent mode"
            )
        else:
            try:
                silent_guard = WabbleSpecMemoryConfig().hook_silent_save
...
def hook_precompact(data: dict, harness: str):
    ...
    # Mine WABBLESPEC_MEMORY_DIR synchronously so project data lands before
