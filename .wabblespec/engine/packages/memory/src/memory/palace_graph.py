"""
palace_graph.py — Graph traversal layer for WabbleSpec Memory
======================================================
...
from .config import WabbleSpecMemoryConfig, normalize_wing_name
...
logger = logging.getLogger("wabblespec_memory_graph")
...
def _normalize_wing(wing: str | None) -> str | None:
    ...
    (e.g. ``wabblespec_memory_public``).  Callers that pass the raw directory name
    (``wabblespec_memory-public``) would silently miss.  This helper aligns the lookup
...
def _get_collection(config=None):
    config = config or WabbleSpecMemoryConfig()
    try:
...
#
# Stored as a JSON file based on WabbleSpecMemoryConfig.palace_path (where the
...
def _get_tunnel_file(config=None) -> str:
    """Return the path to the tunnels.json file, derived from WabbleSpecMemoryConfig.palace_path."""
    config = config or WabbleSpecMemoryConfig()
...
def _legacy_tunnel_file() -> str:
    ...
    return os.path.join(os.path.expanduser("~"), ".wabblespec_memory", "tunnels.json")
...
def _load_tunnels(config=None):
    ...
    ``~/.wabblespec_memory/tunnels.json`` regardless of the configured palace_path.
    ...
    ``WabbleSpecMemoryConfig`` (which re-reads ``memory.yaml`` from disk) on
    ...
    if os.path.exists(current_tunnel_file):
        try:
            ...
            logger.warning(
                "WabbleSpecMemory tunnels file '%s' is corrupt or unreadable; starting empty.",
                current_tunnel_file,
            )
            return []
...
def _save_tunnels(tunnels, config=None):
    ...
    ``config`` may be passed in by the caller to avoid re-instantiating
    ``WabbleSpecMemoryConfig`` on every save.
    """
...
def create_tunnel(
    ...
    # Single WabbleSpecMemoryConfig() per call — reused by _get_tunnel_file /
    # _load_tunnels / _save_tunnels below. Each WabbleSpecMemoryConfig() re-reads
    # memory.yaml from disk; before this change the helpers each
...
def follow_tunnels(wing: str, room: str, col=None, config=None):
    ...
    # memory.yaml slug (underscore) and an explicit ``--wing`` slug
...
# ``~/.wabblespec_memory/known_entities.json`` under ``topics_by_wing``.
#
...
def entity_tunnels_for_wing(
    ...
    same storage path (``create_tunnel`` → ``~/.wabblespec_memory/tunnels.json``),
