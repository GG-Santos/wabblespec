...
def _strip_leaked_pythonpath_from_sys_path() -> None:
    ...
    # (memory.cli:main, memory.mcp_server:main) drop PYTHONPATH from
    # the env themselves so any subprocess they spawn starts clean. Host
    # applications that embed memory as a library (e.g. import
    # memory.searcher) keep their PYTHONPATH intact for their own
