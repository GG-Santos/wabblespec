...
3. **Benchmark schema** at `.wabblespec/engine/shared/schemas/benchmark.schema.json` — must be present and valid
...
**Benchmark** failure halts promotion and appends to `.wabblespec/engine/shared/references/benchmark-tracker.json`. The promotion does not retry automatically — a new Synth-Blueprint cycle is required.
