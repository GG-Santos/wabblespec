...
Both operations are optional enrichment. If context7 is unavailable, execution proceeds with available spec artifacts and `.wabblespec/engine/shared/dev/` reference guides. Progress never gates on context7 availability.
...
- That library is not covered by the relevant `.wabblespec/engine/shared/dev/` reference guide
...
- The `.wabblespec/engine/shared/dev/` guide covers the library adequately
