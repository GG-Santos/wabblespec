...
How to write `.wabblespec/engine/shared/references/` and `.wabblespec/engine/shared/dev/` files for WabbleSpec. Consumers: skill-factory, any framework author.

---

## When this applies

When creating or editing any file in `.wabblespec/engine/shared/references/` or `.wabblespec/engine/shared/dev/`. These files are read by modules during cold-start with no prior session context. Every authoring decision affects how reliably a cold agent can find, load, and apply the file.
...
```markdown
Full rules: `.wabblespec/engine/shared/references/staleness-states.md`
Schema: `.wabblespec/engine/shared/schemas/drawer.schema.json`
```
