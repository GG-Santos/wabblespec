...
Usage:
    ...
    python _shared/scripts/session-state.py init --session-id seed-run-20260526xx

    # Show current state:
    python _shared/scripts/session-state.py show

    # Activate enforcement for a module:
    python _shared/scripts/session-state.py set \\
    ...
    python _shared/scripts/session-state.py complete \\
        --task "Build foo.md — ARCHIVE PASS"

    # Clear session (enforcement off, no active module):
    python _shared/scripts/session-state.py clear

    # Check if enforcement is currently active (exit 0 = yes, 1 = no):
    python _shared/scripts/session-state.py check --enforcement-active
