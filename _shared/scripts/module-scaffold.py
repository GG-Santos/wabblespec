...
Usage:
    python _shared/scripts/module-scaffold.py \\
        --id my-module \\
        --layer L5 \\
    ...
    python _shared/scripts/module-scaffold.py --id my-module --layer L5 \\
...
fields required (see `_shared/schemas/receipt.base.schema.json`). Status PASS =
...
- [TODO: list error types from _shared/references/error-taxonomy.md this module can emit, or "(none)"]
"""
...
no recovery path applies, surface a typed error from `_shared/references/error-taxonomy.md`.
"""
...
def main():
    ...
    if args.register and not args.dry_run:
        ...
        if result.returncode == 0:
            ...
            print("Run manually: python _shared/scripts/framework-register.py register ...")
    ...
    print(f"  4. Run: python _shared/scripts/quality-floor-check.py --module {args.id}")
    if not args.register:
        print(f"  5. Register: python _shared/scripts/framework-register.py register --id {args.id} --layer {args.layer}")
