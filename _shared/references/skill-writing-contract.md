...
regression checks. The module auditor (`_shared/agents/module-auditor.md`)
uses this reference as its scoring rubric. The quality floor check
(`_shared/scripts/quality-floor-check.py`) enforces Gate 1 and Gate 2;
...
The quality floor gates (`_shared/references/quality-floor-gates.md`) are
...
5. Run `_shared/agents/module-auditor.md` before marking `build_complete: true`.
