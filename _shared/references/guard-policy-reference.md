...
Classifies shell commands against `_shared/references/command-risk-policy.md`.
...
All error events conform to `_shared/schemas/error-event.schema.json`. See `_shared/references/error-event-catalog.md` for per-error message templates.
...
- `_shared/references/invariants.md` — full invariant definitions (I1–I12 and Memory variants)
- `_shared/references/command-risk-policy.md` — SAFE/WARN/BLOCK command classification table
- `_shared/references/error-event-catalog.md` — per-module error event registry with user-facing message templates
- `_shared/schemas/error-event.schema.json` — typed error event schema
- `_shared/schemas/skill-rules.schema.json` — authority declaration schema
