...
schema = json.loads(pathlib.Path('_shared/schemas/receipt.base.schema.json').read_text())
...
2. Read `_shared/references/invariants.md` for that invariant's definition
...
**Fix path:**
1. If hook file missing: restore from `_shared/hooks/` or re-implement per hook specification
