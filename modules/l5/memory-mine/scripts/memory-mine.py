...
def find_palace_path() -> Path:
    env_path = os.environ.get("MEMPALACE_PALACE_PATH")
    if env_path:
        return Path(env_path)
    for p in [Path.cwd(), *Path.cwd().parents]:
        if (p / ".wabblespec").is_dir():
            return p / ".wabblespec" / "memory"
    raise EnvironmentError(
        "MEMPALACE_PALACE_PATH not set and no .wabblespec directory found. "
        "Run scripts/wabblespec-mempalace-bootstrap.py first."
    )
...
def load_all_drawers(palace_path: Path) -> list[dict]:
    ...
    palace = Palace(str(palace_path))
    try:
        drawers = palace.filter_drawers(limit=10000)
    except Exception as e:
        print(json.dumps({"error": f"Palace.filter_drawers failed: {e}"}))
        sys.exit(1)
