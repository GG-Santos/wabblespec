...
All receipts follow the base schema at `_shared/schemas/receipt.base.schema.json`.
...
**Run:** `python _shared/scripts/quality-floor-check.py --framework framework.yaml`
...
3. **Benchmark schema** — `_shared/schemas/benchmark.schema.json` must be present and valid
...
**Run:** `python _shared/scripts/validate-graph.py --framework framework.yaml`
