...
|---|---|---|
| Build target (platform) | L3 Platform SKILL.md `capability_handoff` | `_shared/dev/frameworks/{platform}/` files |
...
- Shared artifacts (`_shared/`) are consumed by any module; consumers are declared in `framework.yaml` under `shared_files[].consumers`.
...
Full invariant list: `_shared/references/invariants.md`.
...
| `_shared/infrastructure/` | Cross-module policy docs (gateway pattern, guard policy, economy, version tracking) |
| `_shared/schemas/` | Shared JSON schemas (receipt base, etc.) |
| `_shared/scripts/` | Analysis and validation scripts |
| `_shared/templates/` | Shared output templates |
