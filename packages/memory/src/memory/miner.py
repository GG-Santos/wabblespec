...
Reads memory.yaml from the project directory to know the wing + rooms.
...
# ``memory.miner.compute_hallways_for_wing``. The integration call
...
# ``memory.miner`` keep working unchanged. Single source of truth
...
# this via `WABBLESPEC_MEMORY_MAX_CHUNKS_PER_FILE` or
# `memory mine --max-chunks-per-file N` if you hit ONNX bad_alloc on
...
def _resolve_max_chunks_per_file(override: Optional[int] = None) -> int:
    ...
    Precedence: ``override`` (CLI flag) > ``WABBLESPEC_MEMORY_MAX_CHUNKS_PER_FILE``
    ...
    if override is not None:
        ...
        return int(override)
    raw = os.environ.get("WABBLESPEC_MEMORY_MAX_CHUNKS_PER_FILE")
    if raw is None:
        return MAX_CHUNKS_PER_FILE
    try:
        val = int(raw)
        ...
        print(
            f"  ! WARNING: WABBLESPEC_MEMORY_MAX_CHUNKS_PER_FILE={raw!r} is not an integer; "
            f"using default {MAX_CHUNKS_PER_FILE}",
            file=sys.stderr,
        )
        return MAX_CHUNKS_PER_FILE
    if val < 0:
        print(
            f"  ! WARNING: WABBLESPEC_MEMORY_MAX_CHUNKS_PER_FILE={val} is negative; "
            f"using default {MAX_CHUNKS_PER_FILE}",
            file=sys.stderr,
        )
        return MAX_CHUNKS_PER_FILE
...
def load_config(project_dir: str) -> dict:
    """Load memory.yaml from project directory (falls back to mempal.yaml)."""
    import yaml

    resolved_project_dir = Path(project_dir).expanduser().resolve()
    config_path = resolved_project_dir / "memory.yaml"
...
def chunk_text(
    ...
    # Defensive invariant guard. ``WabbleSpecMemoryConfig.chunk_*`` already
...
_ENTITY_REGISTRY_PATH = os.path.join(
    os.environ.get(
        "WABBLESPEC_MEMORY_PATH",
        os.path.join(os.path.expanduser("~"), ".wabblespec_memory"),
    ),
    "known_entities.json",
)
...
def _refresh_known_entities_cache() -> None:
    """Reload ``~/.wabblespec_memory/known_entities.json`` into the module cache if
...
def add_to_known_entities(entities_by_category: dict, wing: str = None) -> str:
    """Union ``entities_by_category`` into ``~/.wabblespec_memory/known_entities.json``.

    Accepts ``{category: [names]}`` shape as produced by ``memory init``
...
def detect_hall(content: str) -> str:
    ...
    if _HALL_KEYWORDS_CACHE is None:
        ...
        _HALL_KEYWORDS_CACHE = WabbleSpecMemoryConfig().hall_keywords
...
def process_file(
    ...
    if effective_cap > 0 and len(chunks) > effective_cap:
        ...
        # line). This keeps ``memory mine ... > out.log 2> err.log``
        ...
        print(
            ...
            f"WABBLESPEC_MEMORY_MAX_CHUNKS_PER_FILE (set 0 to disable), or add to "
...
def mine(
    ...
    ``WABBLESPEC_MEMORY_MAX_CHUNKS_PER_FILE`` or ``MAX_CHUNKS_PER_FILE``; ``0``
...
def _mine_impl(
    ...
    palace_config = WabbleSpecMemoryConfig()
    ...
    try:
        ...
        if files_skipped_chunk_cap > 0:
            ...
            print(
                ...
                f"(raise via --max-chunks-per-file or WABBLESPEC_MEMORY_MAX_CHUNKS_PER_FILE; "
                f"set 0 to disable)"
            )
        ...
        print('\n  Next: run MemorySearch for what you need')
        print(f"{'=' * 55}\n")
    except KeyboardInterrupt:
        ...
        print(
            f"\n  Re-run `memory mine {shlex.quote(project_dir)}` to resume — "
        ...
        print(
            f"\n  Re-run `memory mine {shlex.quote(project_dir)}` after addressing "
...
def _cleanup_mine_pid_file() -> None:
    ...
    Hook-spawned mines receive ``WABBLESPEC_MEMORY_MINE_PID_FILE`` in their env
    pointing at the slot the hook claimed for them
    (``~/.wabblespec_memory/hook_state/mine_pids/mine_<sha>.pid``). When the
    ...
    """
    pid_file_env = os.environ.get("WABBLESPEC_MEMORY_MINE_PID_FILE", "")
    if not pid_file_env:
        return
    try:
...
def _compute_entity_tunnels_for_wing(wing: str) -> int:
    ...
    Reads hallway records (``memory.hallways.list_hallways``) and
...
def status(palace_path: str):
    ...
    print(f"\n{'=' * 55}")
    print(f"  WabbleSpec Memory Status — {total} drawers")
    print(f"{'=' * 55}\n")
