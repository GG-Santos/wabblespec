#!/usr/bin/env python3
"""
WabbleSpec Memory MCP Server — read/write palace access for Claude Code
================================================================
Install from this checkout:
  claude mcp add memory -- python scripts/memory-mcp.py [--palace /path/to/palace]

Compatibility note: the server/package command is now ``memory`` /
``memory-mcp``, but MCP tool identifiers intentionally remain
``wabblespec_memory_*`` so existing clients, saved prompts, and instructions
continue to work.

Tools (read):
  wabblespec_memory_status          — total drawers, wing/room breakdown
  wabblespec_memory_list_wings      — all wings with drawer counts
  wabblespec_memory_list_rooms      — rooms within a wing
  wabblespec_memory_get_taxonomy    — full wing → room → count tree
  wabblespec_memory_search          — semantic search, optional wing/room filter
  wabblespec_memory_check_duplicate — check if content already exists before filing

Tools (write):
  wabblespec_memory_add_drawer      — file verbatim content into a wing/room
  wabblespec_memory_delete_drawer   — remove a drawer by ID

Tools (maintenance):
  wabblespec_memory_reconnect       — force cache invalidation and reconnect after external writes
"""
...
def _init_logging() -> None:
    """Root-logger init: always stderr, optionally append to ``WABBLESPEC_MEMORY_LOG_FILE``.

    Stderr-only is the default. When ``WABBLESPEC_MEMORY_LOG_FILE`` is set, a
    ...
    * Root logger already configured (host app embedding the server,
      ...
      the handlers so WABBLESPEC_MEMORY_LOG_FILE's contract holds regardless
      ...
      exists to close.
    * Concurrent writers (multiple ``memory-mcp`` processes pointing
    ...
    call. Callers that import ``memory.mcp_server`` for introspection
    ...
    # WABBLESPEC_MEMORY_LOG_FILE is operator-supplied and opt-in; this is a
    # local-first server (CLAUDE.md design principle), so no path
    # sanitization — the operator's process UID is the trust boundary.
    log_file = os.environ.get("WABBLESPEC_MEMORY_LOG_FILE", "").strip()
    ...
    if file_handler_error is not None:
        logging.getLogger("wabblespec_memory_mcp").warning(
            "WABBLESPEC_MEMORY_LOG_FILE=%r could not be opened (%s); using stderr only",
            log_file,
            file_handler_error,
        )
...
_init_logging()
logger = logging.getLogger("wabblespec_memory_mcp")


def _parse_args():
    parser = argparse.ArgumentParser(description="WabbleSpec Memory MCP Server")
    parser.add_argument(
        "--palace",
        metavar="PATH",
...
if _args.palace:
    os.environ["WABBLESPEC_MEMORY_PATH"] = os.path.abspath(_args.palace)

_config = WabbleSpecMemoryConfig()
...
# handles on Windows.  When WABBLESPEC_MEMORY_MCP_IDLE_HOURS is set (or defaults
# to 8 h), a background daemon thread exits the process once no request
# has been handled for that long.  Set to 0 to disable.
_MCP_IDLE_HOURS_ENV = "WABBLESPEC_MEMORY_MCP_IDLE_HOURS"
...
def _get_kg(canonical_path=None) -> KnowledgeGraph:
    ...
    inside this function would let ``WABBLESPEC_MEMORY_PATH`` rotation,
...
# :func:`memory.searcher._bm25_only_via_sqlite`. Cleared after a
    ...
    if info.get("diverged"):
        if not _vector_disabled:
            logger.warning(
                ...
                "BM25-only sqlite fallback. Run `memory repair` to restore "
                "vector search.",
...
_WAL_DIR = Path(
    os.environ.get(
        "WABBLESPEC_MEMORY_WAL_DIR",
        str(Path(_config.palace_path) / ".runtime" / "wal"),
    )
)
...
def _get_collection(create=False):
    ...
    for attempt in range(2):
        try:
            ...
            # one we spoof in ``memory.embedding`` (both report ``"default"``,
...
def _no_palace():
    return {
        ...
        "hint": "Run: memory init <dir> && memory mine <dir>",
    }
...
# Also available via wabblespec_memory_get_aaak_spec tool.

PALACE_PROTOCOL = """IMPORTANT — WabbleSpec Memory Memory Protocol:
1. ON WAKE-UP: Call wabblespec_memory_status to load palace overview + AAAK spec.
2. BEFORE RESPONDING about any person, project, or past event: call wabblespec_memory_kg_query or wabblespec_memory_search FIRST. Never guess — verify.
3. IF UNSURE about a fact (name, gender, age, relationship): say "let me check" and query the palace. Wrong is worse than slow.
4. AFTER EACH SESSION: call wabblespec_memory_diary_write to record what happened, what you learned, what matters.
5. WHEN FACTS CHANGE: call wabblespec_memory_kg_invalidate on the old fact, wabblespec_memory_kg_add for the new one.

This protocol ensures the AI KNOWS before it speaks. Storage is not memory — but storage + this protocol = memory."""

AAAK_SPEC = """AAAK is a compressed memory dialect that WabbleSpec Memory uses for efficient storage.
...
def tool_check_duplicate(content: str, threshold: float = 0.9):
    ...
    if _vector_disabled:
        ...
        return {
            ...
            "vector_disabled_reason": _vector_disabled_reason,
            "hint": (
                "duplicate detection requires vector search; run `memory repair` to restore"
            ),
        }
    col = _get_collection()
    if not col:
...
def tool_diary_read(agent_name: str, last_n: int = 10, wing: str = ""):
    ...
    use ``memory repair`` to migrate legacy data if needed.
    """
    try:
        agent_name = sanitize_name(agent_name, "agent_name").lower()
        if wing:
...
def tool_hook_settings(silent_save: bool = None, desktop_toast: bool = None):
    ...
    """
    from .config import WabbleSpecMemoryConfig

    try:
        config = WabbleSpecMemoryConfig()
    except Exception as e:
        return {"success": False, "error": str(e)}
...
def tool_memories_filed_away():
    ...
    state_dir = Path.home() / ".wabblespec_memory" / "hook_state"
...
TOOLS = {
    "wabblespec_memory_status": {
        "description": "Palace overview — total drawers, wing and room counts",
        "input_schema": {"type": "object", "properties": {}},
        "handler": tool_status,
    },
    "wabblespec_memory_list_wings": {
        "description": "List all wings with drawer counts",
        "input_schema": {"type": "object", "properties": {}},
        "handler": tool_list_wings,
    },
    "wabblespec_memory_list_rooms": {
    ...
    "wabblespec_memory_get_taxonomy": {
        "description": "Full taxonomy: wing → room → drawer count",
        "input_schema": {"type": "object", "properties": {}},
        "handler": tool_get_taxonomy,
    },
    "wabblespec_memory_get_aaak_spec": {
        "description": "Get the AAAK dialect specification — the compressed memory format WabbleSpec Memory uses. Call this if you need to read or write AAAK-compressed memories.",
        "input_schema": {"type": "object", "properties": {}},
        "handler": tool_get_aaak_spec,
    },
    "wabblespec_memory_kg_query": {
    ...
    "wabblespec_memory_kg_invalidate": {
    ...
    "wabblespec_memory_kg_stats": {
        "description": "Knowledge graph overview: entities, triples, current vs expired facts, relationship types.",
        "input_schema": {"type": "object", "properties": {}},
        "handler": tool_kg_stats,
    },
    "wabblespec_memory_traverse": {
    ...
    "wabblespec_memory_find_tunnels": {
    ...
    "wabblespec_memory_graph_stats": {
        "description": "Palace graph overview: total rooms, tunnel connections, edges between wings.",
        "input_schema": {"type": "object", "properties": {}},
        "handler": tool_graph_stats,
    },
    "wabblespec_memory_create_tunnel": {
    ...
    "wabblespec_memory_list_tunnels": {
    ...
    "wabblespec_memory_delete_tunnel": {
    ...
    "wabblespec_memory_follow_tunnels": {
    ...
    "wabblespec_memory_check_duplicate": {
    ...
    "wabblespec_memory_delete_drawer": {
    ...
    "wabblespec_memory_list_drawers": {
    ...
    "wabblespec_memory_update_drawer": {
    ...
    "wabblespec_memory_hook_settings": {
        "description": (
    ...
    "wabblespec_memory_memories_filed_away": {
    ...
    if method == "initialize":
        ...
        return {
            ...
            "result": {
                ...
                "capabilities": {"tools": {}},
                "serverInfo": {"name": "wabblespec_memory", "version": __version__},
            },
        }
    elif method == "ping":
...
# InferenceSession.
_WARMUP_PROBE_TEXT = "__wabblespec_memory_warmup_probe__"
...
def _maybe_eager_warmup_embedder() -> None:
    """Pre-load embedder + HNSW segment at startup when ``WABBLESPEC_MEMORY_EAGER_WARMUP`` is truthy.
    ...
    1. ONNX/CoreML embedder init in :func:`memory.embedding.get_embedding_function`
    ...
    ``memory init`` (which would violate CLAUDE.md "Incremental only"),
    we test for ``chroma.sqlite3`` ourselves before touching the chromadb
    client. Operators who set ``WABBLESPEC_MEMORY_EAGER_WARMUP=1`` in their MCP
    config and launch the server before running ``memory init`` get a
    ...
    * **Query failure** (network failure during ONNX model download,
    ...
    ``memory init`` but never written to), ``col.query`` succeeds but
    ...
    """
    raw = os.environ.get("WABBLESPEC_MEMORY_EAGER_WARMUP", "").strip().lower()
    if raw in _WARMUP_FALSY:
        return
    if raw not in _WARMUP_TRUTHY:
        logger.warning(
            "WABBLESPEC_MEMORY_EAGER_WARMUP=%r is not recognized (use one of %s); warmup disabled",
            raw,
    ...
    if not os.path.isfile(db_path):
        ...
        logger.info(
            "WABBLESPEC_MEMORY_EAGER_WARMUP=%s: no palace at %s — nothing to warm",
            raw,
            palace_path,
        )
        return
    ...
    try:
        ...
        logger.exception(
            "WABBLESPEC_MEMORY_EAGER_WARMUP=%s: collection open failed (palace=%s, device=%s, error=%s)",
            raw,
            palace_path,
            device,
            type(exc).__name__,
    ...
    if col is None:
        logger.warning(
            "WABBLESPEC_MEMORY_EAGER_WARMUP=%s: _get_collection returned None for palace=%s — see prior log lines",
            raw,
            palace_path,
        )
        return
    try:
        ...
        logger.exception(
            "WABBLESPEC_MEMORY_EAGER_WARMUP=%s: warmup query failed (palace=%s, device=%s, error=%s)",
            raw,
            palace_path,
            device,
            type(exc).__name__,
        ...
        logger.info(
            "WABBLESPEC_MEMORY_EAGER_WARMUP=%s: embedder + HNSW ready (palace=%s, device=%s)",
            raw,
            palace_path,
            device,
        )
...
def _start_idle_exit_watchdog() -> None:
    ...
    When no request has been handled for ``WABBLESPEC_MEMORY_MCP_IDLE_HOURS``
    ...
    Set ``WABBLESPEC_MEMORY_MCP_IDLE_HOURS=0`` to disable the watchdog.
    """
...
def main():
    """MCP server entry point for the ``memory-mcp`` console script.
    ...
    (``import memory.searcher`` from a host app) do NOT trigger this
    ...
    # with a clean env. The sys.path filter in wabblespec_memory/__init__.py
    ...
    for stream in (sys.stdin, sys.stdout):
        if hasattr(stream, "reconfigure"):
            try:
                ...
                pass
    logger.info("WabbleSpec Memory MCP Server starting...")
