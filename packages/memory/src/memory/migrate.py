#!/usr/bin/env python3
"""
wabblespec_memory migrate — Recover a palace created with a different ChromaDB version.
...
Since wabblespec_memory 3.2.0 (chromadb>=1.5.4), chromadb automatically migrates
...
older wabblespec_memory release) or if automatic migration fails.

Usage:
    wabblespec_memory migrate                          # migrate default palace
    wabblespec_memory migrate --palace /path/to/palace  # migrate specific palace
    wabblespec_memory migrate --dry-run                # show what would be migrated
"""
...
def contains_palace_database(path: str) -> bool:
    """Return True when path looks like a WabbleSpec Memory ChromaDB directory."""
...
def collection_write_roundtrip_works(col) -> bool:
    ...
    probe_id = f"_wabblespec_memory_migrate_probe_{uuid.uuid4().hex}"
    probe_doc = "wabblespec_memory migrate write round-trip probe"
    probe_meta = {
        "wing": "_wabblespec_memory_probe",
        "room": "_wabblespec_memory_probe",
        "source_file": "wabblespec_memory_migrate_probe",
        "chunk_index": 0,
    }
...
def migrate(palace_path: str, dry_run: bool = False, confirm: bool = False):
    ...
    print(f"\n{'=' * 60}")
    print("  WabbleSpec Memory Migrate")
    print(f"{'=' * 60}\n")
    ...
    try:
        col = ChromaBackend().get_collection(palace_path, "wabblespec_drawers")
        count = col.count()
    ...
    temp_palace = tempfile.mkdtemp(prefix="wabblespec_memory_migrate_")
    try:
        print(f"  Creating fresh palace in {temp_palace}...")
        fresh_backend = ChromaBackend()
        col = fresh_backend.get_or_create_collection(temp_palace, "wabblespec_drawers")
