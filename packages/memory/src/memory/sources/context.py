...
import ``memory.palace`` directly — that coupling is what the facade
exists to prevent.
...
class _CollectionLike(Protocol):
    """Minimum of :class:`memory.backends.BaseCollection` adapters rely on.
    ...
    concrete backend. See ``wabblespec_memory/backends/base.py`` for the full surface.
    """
...
class PalaceContext:
    ...
    Fields:
        ...
        config: Palace config object (hall keywords, rooms list, privacy
            floor, etc.). Shape is the existing :class:`WabbleSpecMemoryConfig`.
