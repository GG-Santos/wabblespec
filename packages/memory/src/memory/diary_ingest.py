...
- Only new entries are processed by default (tracks entry count in a state
  file under ``~/.wabblespec_memory/state/`` — never inside the user's diary dir).
...
Usage:
    python -m memory.diary_ingest --dir ~/daily_summaries --palace ~/.wabblespec_memory/palace
    python -m memory.diary_ingest --dir ~/daily_summaries --palace ~/.wabblespec_memory/palace --force
"""
...
def _state_file_for(palace_path: str, diary_dir: Path) -> Path:
    """Return the per-(palace, diary-dir) state-file path under ~/.wabblespec_memory/state.
    ...
    """
    state_root = Path(os.path.expanduser("~")) / ".wabblespec_memory" / "state"
...
def ingest_diaries(
    ...
    chunk_size = WabbleSpecMemoryConfig().chunk_size
...
if __name__ == "__main__":
    ...
    parser.add_argument("--palace", default=os.path.expanduser("~/.wabblespec_memory/palace"))
