...
``memory.backends`` entry point::

    # pyproject.toml of wabblespec_memory-postgres
    [project.entry-points."memory.backends"]
    postgres = "wabblespec_memory_postgres:PostgresBackend"

WabbleSpec Memory discovers them at process start. In-tree tests and local development
...
_ENTRY_POINT_GROUP = "memory.backends"
...
def resolve_backend_for_palace(
    *,
    ...
    3. ``WABBLESPEC_MEMORY_BACKEND`` env var
