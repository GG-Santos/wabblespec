"""
WabbleSpec Memory configuration system.

Priority: env vars > config file (~/.wabblespec_memory/config.json) > defaults
"""
...
def normalize_wing_name(name: str) -> str:
    ...
    and when writing `memory.yaml`, so the miner's lookup matches at
...
DEFAULT_PALACE_PATH = os.path.expanduser("~/.wabblespec_memory/palace")
DEFAULT_COLLECTION_NAME = "wabblespec_drawers"


@lru_cache(maxsize=1)
def get_configured_collection_name() -> str:
    """Return the configured drawer collection name without repeated config-file reads."""
    return WabbleSpecMemoryConfig().collection_name


# Single source of truth for chunking defaults. ``memory.miner``
# imports these so the legacy module-level ``CHUNK_SIZE`` /
# ``CHUNK_OVERLAP`` / ``MIN_CHUNK_SIZE`` constants stay in sync with
# ``WabbleSpecMemoryConfig.chunk_*``. Putting them here (not in miner.py) keeps
...
class WabbleSpecMemoryConfig:
    """Configuration manager for WabbleSpec Memory.
    ...
    def __init__(self, config_dir=None):
        ...
        Args:
            config_dir: Override config directory (useful for testing).
                        Defaults to ~/.memory.
        """
        env_config_dir = os.environ.get("WABBLESPEC_MEMORY_CONFIG_DIR")
        self._config_dir = Path(
            config_dir or env_config_dir or os.path.expanduser("~/.wabblespec_memory")
        )
    ...
    def palace_path(self):
        """Path to the memory store data directory."""
        env_val = os.environ.get("WABBLESPEC_MEMORY_PATH")
        if env_val:
    ...
    def tunnel_file(self):
        """Path to the tunnel file inside the WabbleSpec memory store."""
        return os.path.join(self.palace_path, "tunnels.json")
    ...
    def entity_languages(self):
        ...
        Reads from env var ``WABBLESPEC_MEMORY_ENTITY_LANGUAGES`` (comma-separated)
        first, then the ``entity_languages`` field in ``config.json``,
        defaulting to ``["en"]``.
        """
        env_val = os.environ.get("WABBLESPEC_MEMORY_ENTITY_LANGUAGES") or os.environ.get(
            "WABBLESPEC_MEMORY_ENTITY_LANGUAGES"
        )
        if env_val:
    ...
    def embedding_device(self):
        ...
        ``"dml"``. Read from env ``WABBLESPEC_MEMORY_EMBEDDING_DEVICE`` first, then
        ``embedding_device`` in ``config.json``, then ``"auto"``.

        ``auto`` resolves to the first available accelerator at runtime via
        :mod:`memory.embedding`; requesting an unavailable accelerator
        logs a warning and falls back to CPU.
        """
        env_val = os.environ.get("WABBLESPEC_MEMORY_EMBEDDING_DEVICE")
        if env_val:
    ...
    def topic_tunnel_min_count(self):
        ...
        link. Reads ``WABBLESPEC_MEMORY_TOPIC_TUNNEL_MIN_COUNT`` env first, then the
        config-file value, then ``1``.
        """
        env_val = os.environ.get("WABBLESPEC_MEMORY_TOPIC_TUNNEL_MIN_COUNT")
        if env_val:
            try:
                parsed = int(env_val)
                if parsed >= 1:
