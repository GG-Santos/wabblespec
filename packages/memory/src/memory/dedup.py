...
Usage (standalone):
    python -m memory.dedup                          # dedup all
    python -m memory.dedup --dry-run                # preview only
    python -m memory.dedup --threshold 0.10         # stricter (near-identical only)
    python -m memory.dedup --threshold 0.35         # looser (catches paraphrased content)
    python -m memory.dedup --wing my_project        # scope to one wing
    python -m memory.dedup --stats                  # stats only
    python -m memory.dedup --source "my_project"    # filter by source

Usage (from CLI):
    memory dedup [--dry-run] [--threshold 0.15] [--stats]
"""
...
def _get_palace_path():
    ...
    try:
        ...
        return os.path.join(os.path.expanduser("~"), ".wabblespec_memory", "palace")
...
def dedup_palace(
    ...
    print(f"\n{'=' * 55}")
    print("  WabbleSpec Memory Deduplicator")
    print(f"{'=' * 55}")
