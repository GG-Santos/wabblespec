...
Usage:
    from memory.fact_checker import check_text
    issues = check_text("Bob is Alice's brother", palace_path)

    # CLI
    python -m memory.fact_checker "Bob is Alice's brother" \\
        --palace ~/.wabblespec_memory/palace
"""
...
# ~/.wabblespec_memory/known_entities.json on every check_text call.
...
def _reconfigure_stdio_utf8_on_windows():
    ...
    Thin wrapper around the shared helper in ``memory._stdio``. Mirrors
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Check text against known facts in the WabbleSpec Memory palace.",
    ...
    parser.add_argument(
        "--palace",
        default=os.path.expanduser("~/.wabblespec_memory/palace"),
