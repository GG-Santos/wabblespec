"""format_miner.py — proposed for wabblespec_memory 3.3.6.
...
Architecture matches the existing miner-per-content-type pattern:
    ...
    memory mine <dir>                  → miner.py
    memory mine <dir> --mode convos    → convo_miner.py
    memory mine <dir> --mode extract   → format_miner.py (this file)
...
If the user runs ``memory mine --mode extract`` without it installed,
...
# memory.format_miner.<name>. Lazy imports inside functions would not
...
logger = logging.getLogger("wabblespec_memory_format_miner")
...
def scan_formats(directory: Union[Path, str]) -> list[Path]:
    ...
    # CLI inputs like ``memory mine --mode extract ~/docs`` work without
...
def mine_formats(
    ...
    Notes
    -----
    - Loads ``WabbleSpecMemoryConfig`` once at the start. The current chunker
    ...
    """
    # WabbleSpecMemoryConfig + chunk_text are imported at module level (above) so
    # tests can patch ``memory.format_miner.WabbleSpecMemoryConfig`` and
    # ``memory.format_miner.chunk_text`` directly. Hoisted as part of the
    ...
    # Load the project's memory.yaml (rooms list + wing override) so
    ...
    try:
        ...
        print(f"\n{'=' * 55}")
        print("  WabbleSpec Memory Mine — Format extraction")
        print(f"{'=' * 55}")
        print(f"  Wing:    {wing}")
        ...
        for i, filepath in enumerate(files, 1):
            ...
            try:
                ...
                # Thread the user's WabbleSpecMemoryConfig chunk parameters through
