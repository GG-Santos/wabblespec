...
def _runtime_dir(name: str, fallback: str) -> str:
    env_key = {
        "locks": "WABBLESPEC_MEMORY_LOCK_DIR",
        "wal": "WABBLESPEC_MEMORY_WAL_DIR",
        "hook_state": "WABBLESPEC_MEMORY_STATE_DIR",
    }.get(name)
    if env_key:
        configured = os.environ.get(env_key)
        if configured:
            return configured
    memory_root = os.environ.get("WABBLESPEC_MEMORY_PATH")
    if memory_root:
        return os.path.join(memory_root, ".runtime", name)
    return fallback
...
def get_closets_collection(palace_path: str, create: bool = True):
    ...
    return get_collection(palace_path, collection_name="wabblespec_closets", create=create)
...
def _open_collection_or_explain(
    ...
    *,
    ...
    :class:`memory.backends.PalaceNotFoundError` /
    :class:`memory.backends.CollectionNotInitializedError` directly.
    ...
    State C: DB is present but the ``wabblespec_drawers`` collection has
    ...
    if not os.path.isdir(palace_path):
        ...
        emit("  Run: memory init <dir> then memory mine <dir>")
        return None
    ...
    try:
        ...
        emit("  Try: memory repair-status --palace <path>")
        return None
...
def _candidate_entity_words(text: str) -> list:
    ...
    if _CANDIDATE_RX_CACHE is None:
        ...
        patterns = get_entity_patterns(WabbleSpecMemoryConfig().entity_languages)
        rxs = []
...
def _build_date_line_segment(drawer_metas):
    ...
    # memory.miner._extract_content_date) when present. Fall back to
...
def mine_lock(source_file: str):
    ...
    lock_dir = _runtime_dir(
        "locks",
        os.path.join(os.path.expanduser("~"), ".wabblespec_memory", "locks"),
    )
...
class MineAlreadyRunning(RuntimeError):
    """Raised when another `memory mine` already holds the per-palace lock."""
...
def mine_palace_lock(palace_path: str):
    ...
    single source; it does not prevent N copies of `memory mine <dir>`
