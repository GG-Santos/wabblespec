...
Usage (standalone):
    python -m memory.repair status
    python -m memory.repair scan [--wing X]
    python -m memory.repair prune --confirm
    python -m memory.repair rebuild

Usage (from CLI):
    memory repair
    memory repair-scan [--wing X]
    memory repair-prune --confirm
"""
...
COLLECTION_NAME = "wabblespec_drawers"
...
CLOSETS_COLLECTION_NAME = "wabblespec_closets"
...
def _drawers_collection_name() -> str:
    ...
    Recovery flows must honor ``WabbleSpecMemoryConfig().collection_name`` so a
    ...
    try:
        ...
        return WabbleSpecMemoryConfig().collection_name or COLLECTION_NAME
    except Exception:
...
def _get_palace_path():
    ...
    try:
        ...
        return WabbleSpecMemoryConfig().palace_path
    except Exception:
        default = os.path.join(os.path.expanduser("~"), ".wabblespec_memory", "palace")
        return default
...
def print_sqlite_integrity_abort(palace_path: str, errors: list[str]) -> None:
    ...
    print("  `memory repair` will not call Chroma delete_collection() because")
    ...
    print("    1. Stop all WabbleSpec Memory writers / MCP clients.")
    ...
    print("    6. Re-run `memory repair --yes`.")
...
def rebuild_index(
    ...
    progress(f"\n{'=' * 55}")
    progress("  WabbleSpec Memory Repair — Index Rebuild")
    progress(f"{'=' * 55}\n")
    progress(f" Memory store: {palace_path}")
    ...
    *,
    ...
    try:
        ...
        for emb_id, doc, meta in extract_via_sqlite(source_palace, collection_name):
            ...
            # non-empty dict). WabbleSpecMemory drawers always carry at least
    ...
    * ``True`` — rename ``dest_palace`` to
    ...
    .. warning::
       ...
       *any* palace, not just this one. Do not call this function from
       inside a long-running wabblespec_memory process (MCP server, daemon)
    ...
    (``string_value`` → ``int_value`` → ``float_value`` → ``bool_value``)
    matches the precedent in :mod:`memory.migrate`. ChromaDB 0.4.x
    ...
    print(f"\n{'=' * 55}")
    print("  WabbleSpec Memory Repair — Rebuild from SQLite")
    print(f"{'=' * 55}\n")
...
def status(palace_path=None, collection_name: Optional[str] = None) -> dict:
    ...
    chromadb tries to load the undersized HNSW. Running ``wabblespec_memory
    ...
    directly via :func:`memory.backends.chroma.hnsw_capacity_status`.
    ...
    collection_name = collection_name or _drawers_collection_name()
    print(f"\n{'=' * 55}")
    print("  WabbleSpec Memory Repair — Status")
    print(f"{'=' * 55}\n")
    ...
    if drawers["diverged"] or closets["diverged"]:
        print("\n  Recommended: run `memory repair` to rebuild the index.")
    print()
...
def repair_max_seq_id(
    ...
    *,
    ...
    print(f"\n{'=' * 55}")
    print("  WabbleSpec Memory Repair — max_seq_id Un-poison")
    print(f"{'=' * 55}\n")
    print(f"  Memory store:  {palace_path}")
    if segment:
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="WabbleSpec Memory repair tools")
