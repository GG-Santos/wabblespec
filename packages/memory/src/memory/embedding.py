...
Supported devices (env ``WABBLESPEC_MEMORY_EMBEDDING_DEVICE`` or ``embedding_device``
in ``~/.wabblespec_memory/config.json``):
...
_DEVICE_PROVIDER_PACKAGE = {
    "cuda": "onnxruntime-gpu",
    "coreml": "an onnxruntime build with CoreMLExecutionProvider",
...
def _resolve_providers(device: str) -> tuple[list, str]:
    ...
    if preferred not in available:
        if device not in _WARNED:
            extra = _DEVICE_PROVIDER_PACKAGE.get(
                device, "the matching onnxruntime provider for your device"
            )
            logger.warning(
...
def _build_ef_class():
    ...
    class _WabbleSpecMemoryONNX(ONNXMiniLM_L6_V2):
        @staticmethod
        def name() -> str:
            return "default"

    return _WabbleSpecMemoryONNX


def get_embedding_function(device: Optional[str] = None):
    """Return a cached embedding function bound to the requested device.

    ``device=None`` reads from :class:`WabbleSpecMemoryConfig.embedding_device`.
    ...
    if device is None:
        ...
        device = WabbleSpecMemoryConfig().embedding_device
