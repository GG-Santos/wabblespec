#!/usr/bin/env python3
"""
entity_registry.py — Persistent personal entity registry for WabbleSpec Memory.
...
def _wikipedia_lookup(word: str) -> dict:
    ...
    **Privacy warning:** This function makes an outbound HTTPS request to
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "WabbleSpec Memory/1.0"})
...
class EntityRegistry:
    ...
    Stored at ~/.wabblespec_memory/entity_registry.json
    Schema:
    {
      ...
      "projects": ["WabbleSpec Memory", "Acme"],
    ...
    DEFAULT_PATH = Path(
        os.environ.get("WABBLESPEC_MEMORY_PATH", str(Path.home() / ".wabblespec_memory"))
    ...
    def learn_from_text(self, text: str, min_confidence: float = 0.75, languages=("en",)) -> list:
        ...
        configured ``WabbleSpecMemoryConfig().entity_languages`` to match the
        locales used at ``memory init`` time.
        """
        from memory.entity_detector import extract_candidates, score_entity, classify_entity
