...
|---|---|
| `package.json` without `bin`, `exports` field, `main`/`module` fields | `_shared/dev/languages/node/` |
| `pyproject.toml` with `[build-system]`, no `[project.scripts]` | `_shared/dev/languages/python/` |
| `lib.rs` or `[lib]` in Cargo.toml | `_shared/dev/languages/rust/` |
| `go.mod`, no `main` package at root | `_shared/dev/languages/go/` |
...
capability_handoff:
  always_load:
    - _shared/dev/frameworks/library/core.md
  conditional_load: []
  gateway_references:
