...
{
  "capability_handoff": {
    "always_load": ["_shared/dev/frameworks/web/react.md", "..."],
    "conditional_load": {
      "signal": "file_or_import_pattern",
      "load": "_shared/dev/frameworks/web/nextjs.md"
    }
  }
}
```
...
4. _shared/dev/infrastructure/ — load if gateway-engineering is active
```
...
When `gateway-engineering` is active, also load from `_shared/dev/infrastructure/`:
- `cicd.md` — always
