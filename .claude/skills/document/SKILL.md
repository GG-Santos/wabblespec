...
**Reference-type outputs require a cross-references section.** When the task-card declares a documentation deliverable of type `reference` — any file destined for `_shared/references/` or explicitly typed `reference` in the task-card — Document must inspect its own output before writing the document receipt.

The inspection check:
```
if deliverable.type == "reference" OR destination starts with "_shared/references/":
...
The check assumes `deliverable.type` is declared in the task-card. Task cards that do not explicitly declare deliverable type use path-based detection (destination starts with `_shared/references/`). Benchmark validated 8 held-out fixture cases (stale_evidence_rate = 0.0).
