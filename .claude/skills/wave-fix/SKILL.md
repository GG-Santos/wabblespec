...
Mark the corresponding quality drawers as SUPERSEDED so Dream removes them from gap-map. Use `drawer-writer.py --update` — do not write drawer JSON inline.

For each fixed HIGH/CRITICAL finding, find the drawer path in `wings/quality/` matching the ref and location:

```bash
python .wabblespec/engine/shared/scripts/drawer-writer.py \
  --update .wabblespec/state/memory/wings/quality/rooms/<ref>/drawers/<drawer-id>.json \
  --staleness-state SUPERSEDED \
  --note "Fixed in wave-fix <timestamp>"
```

Use `drawer-writer.py --list quality/<ref>` to find the matching drawer path before updating.
