...
|---|---|
| `bin` in package.json, `#!/usr/bin/env node` | `_shared/dev/languages/node/` |
| `cli.py`, `__main__.py`, `typer`/`click` import | `_shared/dev/languages/python/` |
| `cmd/` directory, `cobra` import, `go.mod` | `_shared/dev/languages/go/` |
| `clap` in Cargo.toml, Rust `main.rs` with arg parsing | `_shared/dev/languages/rust/` |
...
capability_handoff:
  always_load:
    - _shared/dev/frameworks/cli/core.md
  conditional_load:
    - signal: "cobra import or go.mod"
      load: _shared/dev/frameworks/cli/cobra.md
    - signal: "click or typer import"
      load: _shared/dev/frameworks/cli/click.md
    - signal: "clap in Cargo.toml"
      load: _shared/dev/frameworks/cli/clap.md
  gateway_references:
...
**Platform activation receipt** (`.wabblespec/receipts/platform-cli-{timestamp}.json`):
