...
**Given** an Archive run where files_written across all wave receipts contains at least one framework-graph-relevant file (path starts with `modules/` and ends with `SKILL.md` or `skill-rules.json`; or starts with `_shared/schemas/` and ends with `.json`; or exactly equals `framework.yaml`)
