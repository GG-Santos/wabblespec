---
name: augment
description: Writes experimental implementation of a module change from an approved Blueprint. For AUGMENT type: completes a modified copy of the existing module. For NEW type: completes Factory stubs. All output goes to .wabblespec/experiments/augments/. Never touches production module space.
layer: L8
---
...
3. For NEW type: Factory stubs exist at `.wabblespec/experiments/augments/{blueprint-id}/`
...
| Input | Path | Required |
|---|---|---|
| Approved blueprint | `.wabblespec/experiments/blueprints/{id}.blueprint.json` | Yes |
| Blueprint receipt | `.wabblespec/receipts/blueprint-*.json` | Yes |
| Current module (AUGMENT) | `modules/{layer}/{module}/` | Yes for AUGMENT type |
| Factory stubs (NEW) | `.wabblespec/experiments/augments/{blueprint-id}/` | Yes for NEW type |

## Workflow

**For AUGMENT type:**
1. Copy all files from `modules/{layer}/{module}/` to `.wabblespec/experiments/augments/{blueprint-id}/`
...
**Production contamination** — Augment modifies files in `modules/`. Fix: only `.wabblespec/experiments/augments/` is writable. Any write to `modules/` is a hard violation of I11.
