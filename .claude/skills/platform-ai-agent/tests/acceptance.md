...
Then `_shared/dev/frameworks/ai/langchain.md` is loaded.

## Framework routing: OpenAI SDK

Given `openai` is in dependencies,
When platform-ai-agent detects the framework,
Then `_shared/dev/frameworks/ai/openai-sdk.md` is loaded.
...
Then `_shared/dev/frameworks/ai/core.md` and `_shared/dev/frameworks/ai/safety.md` are always loaded.
